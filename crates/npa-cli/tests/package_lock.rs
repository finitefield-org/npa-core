...
fn package_lock_check_succeeds_on_proof_corpus_fixture_json() {
    ...
    assert_eq!(
        String::from_utf8(output.stdout).unwrap(),
        "{\"schema\":\"npa.package.command_result.v0.5\",\"command\":\"package lock check\",\"root\":\"testdata/package/proofs\",\"status\":\"passed\",\"diagnostics\":[],\"artifacts\":[]}\n"
    );
}
...
fn package_lock_write_creates_missing_lock_and_reports_json() {
    ...
    assert_eq!(
        String::from_utf8(output.stdout).unwrap(),
        "{\"schema\":\"npa.package.command_result.v0.5\",\"command\":\"package lock write\",\"root\":\"<absolute-root>\",\"status\":\"passed\",\"diagnostics\":[],\"artifacts\":[]}\n"
    );
    assert_eq!(fs::read_to_string(lock_path).unwrap(), expected_lock);
}
...
#[test]
fn snapshot_owned_lock_acquisition() {
    package_lock_check_succeeds_on_proof_corpus_fixture();
    package_lock_write_repairs_only_package_lock();
}
