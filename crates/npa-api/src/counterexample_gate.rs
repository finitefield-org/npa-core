...
mod tests {
    ...
    fn checked_profile() -> CounterexampleSearchProfile {
        CounterexampleSearchProfile {
            domain_hash: h(0x10),
            bounds_hash: h(0x11),
            evaluator_identity_hash: h(0x12),
            seed_policy_hash: h(0x13),
            environment_hash: h(0x14),
            finite_carrier: "Bool".to_owned(),
            parameter_range: "all values".to_owned(),
            domain_supported: true,
            evaluator_checked: true,
            decision_procedure_verified: false,
            profile_current: true,
        }
    }

    fn checked_domain(profile: &CounterexampleSearchProfile) -> CounterexampleCheckedDomain {
        CounterexampleCheckedDomain {
            domain_hash: profile.domain_hash,
            bounds_hash: profile.bounds_hash,
            evaluator_identity_hash: profile.evaluator_identity_hash,
            finite_carrier: profile.finite_carrier.clone(),
            parameter_range: profile.parameter_range.clone(),
        }
    }

    fn formal_handoff() -> CounterexampleFormalHandoff {
        CounterexampleFormalHandoff {
            module_name: "Proofs.Ai.Bool".to_owned(),
            theorem_name: "Bool.false_self_eq".to_owned(),
            source_path: "proofs/Proofs/Ai/Bool/source.npa".to_owned(),
            source_hash: h(0x30),
            certificate_path: "proofs/Proofs/Ai/Bool/certificate.npcert".to_owned(),
            certificate_hash: h(0x31),
            meta_path: "proofs/Proofs/Ai/Bool/meta.json".to_owned(),
            meta_hash: h(0x32),
            replay_path: "proofs/Proofs/Ai/Bool/replay.json".to_owned(),
            replay_hash: h(0x33),
            source_free_verification_hash: h(0x34),
        }
    }

    fn result_with_status(status: CounterexampleGateStatus) -> CounterexampleResult {
        let search_profile = checked_profile();
        let search_profile_hash = counterexample_search_profile_hash(&search_profile);
        let mut result = CounterexampleResult {
            api_version: COUNTEREXAMPLE_RESULT_API_VERSION.to_owned(),
            result_key: "cex-result".to_owned(),
            target_key: "target".to_owned(),
            statement_hash: h(0x20),
            status,
            search_profile,
            search_profile_hash,
            evidence_level: ResearchEvidenceLevel::E1,
            candidate_proof_action: CounterexampleGateProofAction::Continue,
            proof_status_advanced: false,
            witness_artifact_hash: None,
            checked_domain: None,
            formal_counterexample_certificate_hash: None,
            formal_counterexample_handoff: None,
            result_hash: h(0xff),
            display_text: None,
        };
        match status {
            CounterexampleGateStatus::Found => {
                result.candidate_proof_action = CounterexampleGateProofAction::Stop;
                result.witness_artifact_hash = Some(h(0x21));
                result.checked_domain = Some(checked_domain(&result.search_profile));
            }
            CounterexampleGateStatus::Unsupported => {
                result.search_profile.domain_supported = false;
                result.search_profile.evaluator_checked = false;
                result.search_profile_hash =
                    counterexample_search_profile_hash(&result.search_profile);
                result.evidence_level = ResearchEvidenceLevel::E0;
            }
            CounterexampleGateStatus::NotFoundWithinBound
            | CounterexampleGateStatus::Inconclusive => {}
        }
        result.result_hash = counterexample_result_hash(&result);
        result
    }

    fn assert_validation_kind(
        result: &CounterexampleResult,
        expected: CounterexampleGateValidationErrorKind,
    ) {
        assert_eq!(
            validate_counterexample_gate(result).map_err(|error| error.kind().clone()),
            Err(expected)
        );
    }
    ...
    fn fixture(name: &str) -> String {
        std::fs::read_to_string(fixture_path(name)).expect("counterexample fixture should exist")
    }

    fn validate_fixture(name: &str) -> Result<(), CounterexampleGateValidationErrorKind> {
        let result = parse_counterexample_result(&fixture(name)).expect("fixture should parse");
        validate_counterexample_gate(&result).map_err(|error| error.kind().clone())
    }

    #[test]
    fn counterexample_gate_requires_checked_evaluator() {
        let found = result_with_status(CounterexampleGateStatus::Found);
        validate_counterexample_gate(&found).expect("checked found witness should pass");
        assert!(matches!(
            found.candidate_proof_action,
            CounterexampleGateProofAction::Defer | CounterexampleGateProofAction::Stop
        ));

        let mut unchecked = found.clone();
        unchecked.search_profile.evaluator_checked = false;
        unchecked.search_profile.decision_procedure_verified = false;
        unchecked.search_profile_hash =
            counterexample_search_profile_hash(&unchecked.search_profile);
        unchecked.result_hash = counterexample_result_hash(&unchecked);
        assert_validation_kind(
            &unchecked,
            CounterexampleGateValidationErrorKind::UncheckedEvaluator {
                status: CounterexampleGateStatus::Found,
            },
        );

        let mut missing_witness = found.clone();
        missing_witness.witness_artifact_hash = None;
        missing_witness.result_hash = counterexample_result_hash(&missing_witness);
        assert_validation_kind(
            &missing_witness,
            CounterexampleGateValidationErrorKind::FoundRequiresWitnessArtifact,
        );

        let mut unsupported_domain = found.clone();
        unsupported_domain.search_profile.domain_supported = false;
        unsupported_domain.search_profile_hash =
            counterexample_search_profile_hash(&unsupported_domain.search_profile);
        unsupported_domain.result_hash = counterexample_result_hash(&unsupported_domain);
        assert_validation_kind(
            &unsupported_domain,
            CounterexampleGateValidationErrorKind::UnsupportedDomainCannotProduceSearchResult {
                status: CounterexampleGateStatus::Found,
        ...
        let mut stale = found;
        stale.search_profile.profile_current = false;
        stale.search_profile_hash = counterexample_search_profile_hash(&stale.search_profile);
        stale.result_hash = counterexample_result_hash(&stale);
        assert_validation_kind(
            &stale,
            CounterexampleGateValidationErrorKind::StaleSearchProfile,
        );
    ...
    #[test]
    fn counterexample_gate_not_found_is_not_proof() {
        let not_found = result_with_status(CounterexampleGateStatus::NotFoundWithinBound);
        validate_counterexample_gate(&not_found)
            .expect("bounded miss should pass as E1 search evidence");

        let mut proof_claim = not_found.clone();
        proof_claim.evidence_level = ResearchEvidenceLevel::E4;
        proof_claim.result_hash = counterexample_result_hash(&proof_claim);
        assert_validation_kind(
            &proof_claim,
            CounterexampleGateValidationErrorKind::NotFoundWithinBoundIsNotProof,
        );

        let mut proof_status = not_found.clone();
        proof_status.proof_status_advanced = true;
        proof_status.result_hash = counterexample_result_hash(&proof_status);
        assert_validation_kind(
            &proof_status,
            CounterexampleGateValidationErrorKind::GateResultCannotAdvanceProofStatus,
        );

        let mut stopped = not_found.clone();
        stopped.candidate_proof_action = CounterexampleGateProofAction::Stop;
        stopped.result_hash = counterexample_result_hash(&stopped);
        assert_validation_kind(
            &stopped,
            CounterexampleGateValidationErrorKind::NonFoundCannotStopProofSearch,
        );

        validate_counterexample_gate(&result_with_status(CounterexampleGateStatus::Unsupported))
            .expect("unsupported is a valid non-proof gate status");
        validate_counterexample_gate(&result_with_status(CounterexampleGateStatus::Inconclusive))
            .expect("inconclusive is a valid non-proof gate status");
    }

    #[test]
    fn formal_counterexample_certificate_requires_checked_handoff() {
        let mut found = result_with_status(CounterexampleGateStatus::Found);
        let handoff = formal_handoff();
        found.formal_counterexample_certificate_hash = Some(handoff.certificate_hash);
        found.formal_counterexample_handoff = Some(handoff);
        found.result_hash = counterexample_result_hash(&found);
        validate_counterexample_gate(&found).expect("formal handoff should pass");

        let mut traversal_path = found.clone();
        traversal_path
            .formal_counterexample_handoff
            .as_mut()
            .expect("handoff should be present")
            .source_path = "proofs/../Bool/source.npa".to_owned();
        traversal_path.result_hash = counterexample_result_hash(&traversal_path);
        assert_validation_kind(
            &traversal_path,
            CounterexampleGateValidationErrorKind::InvalidHandoffPath {
                field: "source_path",
                path: "proofs/../Bool/source.npa".to_owned(),
                expected_suffix: "/source.npa",
            },
        );

        let mut missing_handoff = found.clone();
        missing_handoff.formal_counterexample_handoff = None;
        missing_handoff.result_hash = counterexample_result_hash(&missing_handoff);
        assert_validation_kind(
            &missing_handoff,
            CounterexampleGateValidationErrorKind::FormalCounterexampleRequiresHandoff,
        );
    ...
    #[test]
    fn counterexample_gate_fixtures_cover_positive_and_negative_cases() {
        for name in [
            "found-formal-counterexample-handoff.json",
            "not-found-within-bound.json",
            "unsupported.json",
        ] {
            validate_fixture(name).unwrap_or_else(|error| panic!("{name} should pass: {error:?}"));
        }

        assert_eq!(
            validate_fixture("unchecked-evaluator.json"),
            Err(CounterexampleGateValidationErrorKind::UncheckedEvaluator {
                status: CounterexampleGateStatus::Found,
            })
        );
        assert_eq!(
            validate_fixture("stale-search-profile.json"),
            Err(CounterexampleGateValidationErrorKind::StaleSearchProfile)
        );
        assert_eq!(
            validate_fixture("bound-exhaustion-as-proof.json"),
            Err(CounterexampleGateValidationErrorKind::NotFoundWithinBoundIsNotProof)
        );
        assert_eq!(
            validate_fixture("unsupported-domain-found.json"),
            Err(
                CounterexampleGateValidationErrorKind::UnsupportedDomainCannotProduceSearchResult {
                    status: CounterexampleGateStatus::Found,
                }
        ...
        assert!(matches!(
            parse_counterexample_result(&fixture("malformed-witness.json"))
                .map_err(|error| error.kind().clone()),
            Err(CounterexampleResultSchemaErrorKind::InvalidHash { value })
                if value == "sha256:not-a-valid-witness-artifact-hash"
        ));
    }
}
