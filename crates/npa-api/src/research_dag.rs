...
mod tests {
    ...
    fn fixture(name: &str) -> String {
        std::fs::read_to_string(fixture_path(name)).expect("research DAG fixture should exist")
    }

    fn parse_fixture(name: &str) -> ResearchDag {
        parse_research_dag(&fixture(name)).expect("fixture should parse")
    }

    fn validate_fixture(name: &str) -> Result<(), ResearchDagValidationErrorKind> {
        validate_research_dag(&parse_fixture(name)).map_err(|error| error.kind().clone())
    }

    #[test]
    fn research_dag_validation() {
        let dag = parse_fixture("valid-research-dag.json");
        validate_research_dag(&dag).expect("valid research DAG should pass");

        let node_kinds: BTreeSet<_> = dag.nodes.iter().map(|node| node.node_kind).collect();
        for kind in [
            ResearchDagNodeKind::Definition,
            ResearchDagNodeKind::KnownTheorem,
            ResearchDagNodeKind::EquivalentFormulation,
            ResearchDagNodeKind::Reduction,
            ResearchDagNodeKind::SpecialCase,
            ResearchDagNodeKind::ConditionalLemma,
        ...
        let edge_kinds: BTreeSet<_> = dag.edges.iter().map(|edge| edge.edge_kind).collect();
        for kind in [
            ResearchDagEdgeKind::DependsOn,
            ResearchDagEdgeKind::Implies,
            ResearchDagEdgeKind::EquivalentTo,
            ResearchDagEdgeKind::Strengthens,
            ResearchDagEdgeKind::Weakens,
            ResearchDagEdgeKind::Refutes,
            ResearchDagEdgeKind::SupportedByExperiment,
            ResearchDagEdgeKind::BlockedBy,
        ] {
            assert!(edge_kinds.contains(&kind), "missing edge kind {kind:?}");
        }

        assert_eq!(
            validate_fixture("cycle-research-dag.json"),
            Err(ResearchDagValidationErrorKind::CycleDetected {
                node_key: "node-a".to_owned(),
            })
        );
        assert!(matches!(
            validate_fixture("invalid-evidence-transition.json"),
            Err(
                ResearchDagValidationErrorKind::EvidenceLevelUpgradeRequiresVerifiedArtifact { .. }
            )
        ));
        let mut experiment_upgrade = parse_fixture("invalid-evidence-transition.json");
        experiment_upgrade.edges[0].edge_kind = ResearchDagEdgeKind::SupportedByExperiment;
        assert!(matches!(
            validate_research_dag(&experiment_upgrade).map_err(|error| error.kind().clone()),
            Err(
                ResearchDagValidationErrorKind::EvidenceLevelUpgradeRequiresVerifiedArtifact { .. }
            )
        ));
        let mut checked_counterexample_without_evidence = dag.clone();
        let counterexample_node = checked_counterexample_without_evidence
            .nodes
            .iter_mut()
            .find(|node| node.node_kind == ResearchDagNodeKind::CounterexampleSearch)
            .expect("valid fixture has counterexample search node");
        counterexample_node.evidence_level = ResearchEvidenceLevel::E4;
        counterexample_node.counterexample_or_refutation_hash = None;
        assert_eq!(
            validate_research_dag(&checked_counterexample_without_evidence)
                .map_err(|error| error.kind().clone()),
            Err(
                ResearchDagValidationErrorKind::MissingCounterexampleEvidence {
                    node_key: "counterexample-search".to_owned(),
                }
            )
        );
        assert!(matches!(
            validate_fixture("stale-artifact-reference.json"),
            Err(ResearchDagValidationErrorKind::StaleArtifactReference { .. })
        ));
        assert_eq!(
            validate_fixture("conditional-missing-assumptions.json"),
            Err(ResearchDagValidationErrorKind::MissingAssumptions {
                node_key: "conditional-lemma".to_owned(),
            })
        );
        assert_eq!(
            validate_fixture("unsupported-proof-claim.json"),
            Err(
                ResearchDagValidationErrorKind::SidecarNodeCannotClaimProof {
                    node_key: "candidate-lemma".to_owned(),
                    kind: ResearchDagNodeKind::CandidateLemma,
                }
            )
        );
        assert_eq!(
            validate_fixture("equivalence-missing-review.json"),
            Err(ResearchDagValidationErrorKind::EquivalentRequiresReview {
                edge_key: "equiv-without-review".to_owned(),
            })
        );
        assert_eq!(
            validate_fixture("equivalence-rewrites-target.json"),
            Err(
                ResearchDagValidationErrorKind::EquivalentCannotRewriteTarget {
                    edge_key: "equiv-rewrites-target".to_owned(),
                }
            )
        );

        let mut duplicate_output = dag.clone();
        duplicate_output
            .execution
            .allowed_outputs
            .push(ResearchDagAllowedOutput::ResearchEvidence);
        assert_eq!(
            validate_research_dag(&duplicate_output).map_err(|error| error.kind().clone()),
            Err(ResearchDagValidationErrorKind::DuplicateAllowedOutput {
                output: ResearchDagAllowedOutput::ResearchEvidence,
            })
        );
    }

    #[test]
    fn research_dag_refutes_requires_checked_counterexample() {
        validate_fixture("valid-research-dag.json")
            .expect("valid refutes edge has checked evidence");
        assert_eq!(
            validate_fixture("refutes-without-checked-counterexample.json"),
            Err(
                ResearchDagValidationErrorKind::RefutesRequiresCheckedCounterexample {
                    edge_key: "bad-refutes".to_owned(),
                }
            )
        );
    }
    ...
    #[test]
    fn research_dag_experiment_is_sidecar() {
        let dag = parse_fixture("valid-research-dag.json");
        let experiment = dag
            .nodes
            .iter()
            .find(|node| node.node_kind == ResearchDagNodeKind::ComputationalExperiment)
            .expect("valid fixture includes experiment node");
        assert!(experiment.sidecar_only);
        assert!(!experiment.proof_claim);
        assert!(experiment.experiment_artifact_hash.is_some());

        let barrier = dag
            .nodes
            .iter()
            .find(|node| node.node_kind == ResearchDagNodeKind::BarrierResult)
            .expect("valid fixture includes barrier node");
        assert!(barrier.sidecar_only);
        assert!(!barrier.proof_claim);
        assert!(barrier.barrier_audit_hash.is_some());

        assert_eq!(
            validate_fixture("experiment-as-proof-node.json"),
            Err(
                ResearchDagValidationErrorKind::SidecarNodeCannotClaimProof {
                    node_key: "experiment-node".to_owned(),
                    kind: ResearchDagNodeKind::ComputationalExperiment,
                }
            )
        );
    }
    ...
    #[test]
    fn research_dag_cannot_release_proof_dependencies() {
        assert_eq!(
            validate_fixture("dependency-release-attempt.json"),
            Err(ResearchDagValidationErrorKind::ResearchDagCannotReleaseProofDependency)
        );
    }
}
