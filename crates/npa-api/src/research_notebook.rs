...
mod tests {
    ...
    fn fixture_path(name: &str) -> PathBuf {
        Path::new(env!("CARGO_MANIFEST_DIR"))
            ...
            .join("../npa/develop/proof-using-agents/fixtures/pua-m16-research-notebook")
            .join(name)
    }

    fn fixture(name: &str) -> String {
        std::fs::read_to_string(fixture_path(name)).expect("research notebook fixture should exist")
    }

    fn parse_notebook_fixture(name: &str) -> ResearchNotebookEntry {
        parse_research_notebook_entry(&fixture(name))
            .expect("research notebook fixture should parse")
    }

    fn parse_dead_end_fixture(name: &str) -> DeadEndLedgerRecord {
        parse_dead_end_ledger_record(&fixture(name)).expect("dead-end fixture should parse")
    }

    fn validate_notebook_fixture(name: &str) -> Result<(), ResearchNotebookValidationErrorKind> {
        let entry = parse_notebook_fixture(name);
        validate_research_notebook_entry(&entry).map_err(|error| error.kind().clone())
    }

    fn validate_dead_end_fixture(name: &str) -> Result<(), ResearchNotebookValidationErrorKind> {
        let record = parse_dead_end_fixture(name);
        validate_dead_end_ledger_record(&record).map_err(|error| error.kind().clone())
    }

    #[test]
    fn research_notebook_entry_identity() {
        let entry = parse_notebook_fixture("valid-certificate-reference.json");
        validate_research_notebook_entry(&entry).expect("certificate notebook entry validates");
        assert_eq!(entry.event_index, 7);
        assert_eq!(entry.certificate_references.len(), 1);
        assert_eq!(
            research_notebook_entry_hash(&entry),
            research_notebook_entry_hash(&entry)
        );

        let mut wall_clock_changed = entry.clone();
        wall_clock_changed.wall_clock_time = Some("2099-12-31T23:59:59Z".to_owned());
        wall_clock_changed.display_text = Some("display text changed".to_owned());
        wall_clock_changed.entry_key = "notebook.entry.replayed-copy".to_owned();
        assert_eq!(
            research_notebook_entry_hash(&entry),
            research_notebook_entry_hash(&wall_clock_changed)
        );

        let mut event_changed = entry.clone();
        event_changed.event_index += 1;
        assert_ne!(
            research_notebook_entry_hash(&entry),
            research_notebook_entry_hash(&event_changed)
        );

        let mut payload_changed = entry.clone();
        payload_changed.payload_hash = parse_hash_string(
            "sha256:9999999999999999999999999999999999999999999999999999999999999999",
        )
        .expect("test hash parses");
        assert_ne!(
            research_notebook_entry_hash(&entry),
            research_notebook_entry_hash(&payload_changed)
        );

        let weekly = parse_notebook_fixture("valid-weekly-review.json");
        validate_research_notebook_entry(&weekly).expect("weekly review entry validates");
    }

    #[test]
    fn research_notebook_rejects_proof_claim() {
        assert!(matches!(
            validate_notebook_fixture("notebook-claims-proof.json"),
            Err(ResearchNotebookValidationErrorKind::NotebookCannotClaimProof)
        ));
        assert!(matches!(
            validate_notebook_fixture("stale-dependency-hash.json"),
            Err(ResearchNotebookValidationErrorKind::StaleDependencyHash { .. })
        ));
        assert!(matches!(
            validate_notebook_fixture("missing-redaction.json"),
            Err(ResearchNotebookValidationErrorKind::MissingRedactionReview)
        ));
        assert!(matches!(
            parse_research_notebook_entry(&fixture("certificate-missing-source-free.json")),
            Err(ResearchNotebookSchemaError { .. })
        ));
        assert!(matches!(
            validate_notebook_fixture("notebook-creates-evidence.json"),
            Err(ResearchNotebookValidationErrorKind::NotebookCannotCreateEvidenceRecord)
        ));
        assert!(matches!(
            validate_notebook_fixture("certificate-kind-without-reference.json"),
            Err(ResearchNotebookValidationErrorKind::EntryKindRequiresCertificateReference)
        ));
    }

    #[test]
    fn dead_end_ledger_requires_blocker_kind() {
        let record = parse_dead_end_fixture("valid-dead-end-reviewed.json");
        validate_dead_end_ledger_record(&record).expect("reviewed dead end should validate");
        assert_eq!(
            record.suppression_policy,
            DeadEndSuppressionPolicy::SuppressExactRepeatedRoute
        );
        assert_eq!(
            dead_end_ledger_record_hash(&record),
            dead_end_ledger_record_hash(&record)
        );

        let missing_blocker =
            parse_dead_end_ledger_record(&fixture("dead-end-missing-blocker.json"))
                .expect_err("missing blocker kind should fail schema parsing");
        assert!(matches!(
            missing_blocker.kind(),
            ResearchNotebookSchemaErrorKind::MissingField {
                field: "blocker_kind"
            }
        ));

        assert!(matches!(
            validate_dead_end_fixture("repeated-route-without-blocker-review.json"),
            Err(ResearchNotebookValidationErrorKind::DeadEndCannotSuppressWithoutBlockerReview)
        ));
        assert!(matches!(
            validate_dead_end_fixture("dead-end-stale-dependency.json"),
            Err(ResearchNotebookValidationErrorKind::StaleDependencyHash { .. })
        ));
        assert!(matches!(
            validate_dead_end_fixture("dead-end-blocks-valid-proof-task.json"),
            Err(ResearchNotebookValidationErrorKind::DeadEndCannotBlockValidProofTask)
        ));
        assert!(matches!(
            validate_dead_end_fixture("dead-end-missing-redaction.json"),
            Err(ResearchNotebookValidationErrorKind::MissingRedactionReview)
        ));
        assert!(matches!(
            validate_dead_end_fixture("dead-end-creates-proof-evidence.json"),
            Err(ResearchNotebookValidationErrorKind::DeadEndCannotCreateProofEvidence)
        ));
    }
}
