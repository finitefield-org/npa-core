...
mod tests {
    ...
    fn fixture_path(name: &str) -> PathBuf {
        Path::new(env!("CARGO_MANIFEST_DIR"))
            ...
            .join("../npa/develop/proof-using-agents/fixtures/pua-m16-experiment-artifact")
            .join(name)
    }

    fn fixture(name: &str) -> String {
        std::fs::read_to_string(fixture_path(name))
            .expect("experiment artifact fixture should exist")
    }

    fn parse_fixture(name: &str) -> ExperimentArtifact {
        parse_experiment_artifact(&fixture(name)).expect("experiment artifact fixture should parse")
    }

    fn validate_fixture(name: &str) -> Result<(), ExperimentArtifactValidationErrorKind> {
        let artifact = parse_fixture(name);
        validate_experiment_artifact(&artifact).map_err(|error| error.kind().clone())
    }

    #[test]
    fn experiment_artifact_schema() {
        let bounded = parse_fixture("valid-bounded-reproducible.json");
        assert_eq!(
            bounded.classification,
            ExperimentArtifactClassification::ResearchOnly
        );
        assert_eq!(
            bounded.relationship_to_statement,
            ExperimentRelationshipToStatement::SupportsHypothesis
        );
        validate_experiment_artifact(&bounded).expect("bounded experiment should validate");

        let inconclusive = parse_fixture("valid-inconclusive-nondeterministic.json");
        assert_eq!(
            inconclusive.classification,
            ExperimentArtifactClassification::AuthoringSidecar
        );
        assert_eq!(
            inconclusive.random_seed_policy,
            ExperimentRandomSeedPolicy::NondeterministicDocumentedFailure
        );
        validate_experiment_artifact(&inconclusive)
            .expect("documented nondeterministic failure should validate");

        let malformed = parse_experiment_artifact(&fixture("malformed-hash.json"))
            .expect_err("malformed hash should fail schema parsing");
        assert!(matches!(
            malformed.kind(),
            ExperimentArtifactSchemaErrorKind::InvalidHash { .. }
        ));

        let missing_digest = parse_experiment_artifact(&fixture("missing-container-digest.json"))
            .expect_err("missing container digest should fail schema parsing");
        assert!(matches!(
            missing_digest.kind(),
            ExperimentArtifactSchemaErrorKind::MissingField {
                field: "machine_container_digest"
            }
        ));
    }

    #[test]
    fn experiment_cannot_create_verified_artifact() {
        assert!(matches!(
            validate_fixture("creates-verified-artifact.json"),
            Err(ExperimentArtifactValidationErrorKind::ExperimentCannotCreateVerifiedArtifact)
        ));
        assert!(matches!(
            validate_fixture("claims-proof.json"),
            Err(ExperimentArtifactValidationErrorKind::ExperimentCannotCreateProof)
        ));
        assert!(matches!(
            validate_fixture("general-theorem-output.json"),
            Err(ExperimentArtifactValidationErrorKind::ExperimentCannotCreateGeneralTheorem)
        ));
        assert!(matches!(
            validate_fixture("resolution-evidence-output.json"),
            Err(ExperimentArtifactValidationErrorKind::ExperimentCannotCreateResolutionEvidence)
        ));
        assert!(matches!(
            validate_fixture("dependency-release-output.json"),
            Err(ExperimentArtifactValidationErrorKind::ExperimentCannotReleaseProofDependency)
        ));
        assert!(matches!(
            validate_fixture("claim-gate-success-output.json"),
            Err(ExperimentArtifactValidationErrorKind::ExperimentCannotSatisfyClaimGate)
        ));
        assert!(matches!(
            validate_fixture("retains-secret-context.json"),
            Err(ExperimentArtifactValidationErrorKind::RetentionMayIncludeSecrets)
        ));
        assert!(matches!(
            validate_fixture("retains-raw-prompt-context.json"),
            Err(ExperimentArtifactValidationErrorKind::RetentionMayIncludeRawPrompts)
        ));
        assert!(matches!(
            validate_fixture("retains-unrelated-source-context.json"),
            Err(ExperimentArtifactValidationErrorKind::RetentionMayIncludeUnrelatedSourceContext)
        ));
        assert!(matches!(
            validate_fixture("relationship-output-mismatch.json"),
            Err(ExperimentArtifactValidationErrorKind::RelationshipOutputMismatch { .. })
        ));
    }

    #[test]
    fn experiment_reproducibility_hash() {
        let artifact = parse_fixture("valid-bounded-reproducible.json");
        validate_experiment_artifact(&artifact).expect("reproducible experiment should validate");
        assert_eq!(
            experiment_artifact_hash(&artifact),
            experiment_artifact_hash(&artifact)
        );

        let mut display_changed = artifact.clone();
        display_changed.display_text = Some("human display text changed".to_owned());
        assert_eq!(
            experiment_artifact_hash(&artifact),
            experiment_artifact_hash(&display_changed)
        );

        let mut record_metadata_changed = artifact.clone();
        record_metadata_changed.experiment_key = "exp.pua-m16.rerun-copy".to_owned();
        record_metadata_changed.target_key = "research.target.alias".to_owned();
        record_metadata_changed.classification = ExperimentArtifactClassification::AuthoringSidecar;
        record_metadata_changed.summary_hash = parse_hash_string(
            "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
        )
        .expect("test hash parses");
        assert_eq!(
            experiment_artifact_hash(&artifact),
            experiment_artifact_hash(&record_metadata_changed)
        );

        let mut result_changed = artifact.clone();
        result_changed.result_hash = parse_hash_string(
            "sha256:9999999999999999999999999999999999999999999999999999999999999999",
        )
        .expect("test hash parses");
        assert_ne!(
            experiment_artifact_hash(&artifact),
            experiment_artifact_hash(&result_changed)
        );

        assert!(matches!(
            validate_fixture("nondeterministic-missing-documentation.json"),
            Err(
                ExperimentArtifactValidationErrorKind::NondeterministicSeedPolicyRequiresDocumentation
            )
        ));
        assert!(matches!(
            validate_fixture("deterministic-with-nondeterminism-documentation.json"),
            Err(
                ExperimentArtifactValidationErrorKind::DeterministicSeedPolicyCannotDeclareNondeterminism
            )
        ));
