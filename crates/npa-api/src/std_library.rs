...
const MAX_MACHINE_STD_JSON_BYTES: u64 = 16_777_216;
const MAX_MACHINE_STD_CERTIFICATE_BYTES: u64 = 67_108_864;
...
const STD_CORE_SPEC_ID: &str = "core-spec-v0.2";
const STD_KERNEL_SEMANTICS_PROFILE_ID: &str = "npa-kernel.core.v0.2";
const STD_REDUCTION_PROFILE_ID: &str = "beta-delta-iota.v0.1";
...
const MACHINE_TACTIC_KERNEL_CHECK_PROFILE_TAG: &str = "npa.machine-tactic.kernel-check-profile.v2";
...
/// Producer profile used by the current foundational `npa-std` modules.
///
...
pub const STD_PACKAGE_PRODUCER_PROFILE: &str = "std-library-core-builder";
...
/// Build the deterministic core certificate for a foundational `npa-std` module.
///
/// The current package and the retained legacy package fixture both use the
/// Human/frontend compatibility modules `Std.Logic.Eq` and `Std.Nat.Basic`.
/// This helper exposes only those deterministic builders; it does not read
/// package manifests, source files, registries, or network state.
pub fn build_std_package_module_cert(
    module: &Name,
    imports: &[VerifiedModule],
) -> Option<Result<ModuleCert, CertError>> {
    let core = match module.as_dotted().as_str() {
        "Std.Logic.Eq" => std_logic_eq_core_module(),
        "Std.Nat.Basic" => std_nat_basic_core_module(),
        _ => return None,
    };
    Some(build_module_cert(core, imports))
}

/// Build a foundational certificate for the retained legacy producer-profile
/// fixture.
pub fn build_legacy_std_package_module_cert(
    module: &Name,
    imports: &[VerifiedModule],
) -> Option<Result<ModuleCert, CertError>> {
    build_std_package_module_cert(module, imports)
}

fn std_logic_eq_core_module() -> CoreModule {
    CoreModule {
...
fn std_nat_basic_core_module() -> CoreModule {
    CoreModule {
...
fn read_std_artifact_json(
    ...
    let bytes = read_machine_std_no_follow_file(root, relative_path, MAX_MACHINE_STD_JSON_BYTES)
        .map_err(|source| MachineStdReleaseArtifactError::ReadArtifact {
            artifact,
            path: path.clone(),
            source,
        })?;
    String::from_utf8(bytes).map_err(|source| MachineStdReleaseArtifactError::ReadArtifact {
        artifact,
        path,
...
fn read_optional_std_artifact_json(
    ...
    let path = join_posix_relative_path(root, relative_path);
    match read_machine_std_no_follow_file(root, relative_path, MAX_MACHINE_STD_JSON_BYTES) {
        Ok(bytes) => String::from_utf8(bytes).map(Some).map_err(|source| {
            MachineStdReleaseArtifactError::ReadArtifact {
                artifact,
                path,
                source: io::Error::new(io::ErrorKind::InvalidData, source),
            }
...
fn expr_has_bvar_at_or_above(expr: &Expr, limit: u32) -> bool {
    let mut pending = vec![(expr, Some(limit))];
    let mut seen = HashSet::new();
    while let Some((expr, limit)) = pending.pop() {
        if !seen.insert((std::ptr::from_ref(expr) as usize, limit)) {
            continue;
        }
        match expr {
            Expr::BVar(index) => {
            ...
            Expr::App(fun, arg) => {
                pending.push((arg, limit));
                pending.push((fun, limit));
            }
            Expr::Lam { ty, body, .. } | Expr::Pi { ty, body, .. } => {
                pending.push((body, limit.and_then(|limit| limit.checked_add(1))));
                pending.push((ty, limit));
            }
        }
    }
    false
...
fn syntactic_expr_size(expr: &Expr) -> u64 {
    ...
    while let Some((expr, exiting)) = pending.pop() {
        ...
        let child_size = |child: &Expr| sizes.get(&pointer(child)).copied().unwrap_or(u64::MAX);
        let size = match expr {
            ...
            | Expr::Pi {
                ...
                .saturating_add(child_size(fun))
                .saturating_add(child_size(arg)),
        };
        sizes.insert(key, size);
    }
    sizes.get(&root).copied().unwrap_or(u64::MAX)
}
        ...
        match expr {
            Expr::Const { name, .. } => {
                if let Some(view) = normalize_expr_const_global_ref_view(loaded, owner, name)? {
                    let key =
                        machine_std_global_ref_view_canonical_bytes(&view).map_err(|source| {
                            MachineStdRewriteProfileError::CanonicalBytes { source }
                        })?;
...
fn canonical_package_root(package_root: &Path) -> Result<PathBuf, MachineStdReleaseLoaderError> {
    let canonical = fs::canonicalize(package_root).map_err(|source| {
        MachineStdReleaseLoaderError::InvalidPackageRoot {
            path: package_root.to_path_buf(),
            source,
        }
    })?;
    let metadata = fs::symlink_metadata(package_root).map_err(|source| {
        MachineStdReleaseLoaderError::InvalidPackageRoot {
            path: package_root.to_path_buf(),
            source,
        }
    ...
    if !metadata.file_type().is_dir() || metadata.file_type().is_symlink() {
        return Err(MachineStdReleaseLoaderError::InvalidPackageRoot {
            path: package_root.to_path_buf(),
            source: io::Error::new(
...
fn read_locator_certificate(
    ...
    read_machine_std_no_follow_file(
        package_root,
        &locator.relative_path,
        MAX_MACHINE_STD_CERTIFICATE_BYTES,
    )
    .map_err(|source| {
...
#[cfg(unix)]
fn read_machine_std_no_follow_file(
    root: &Path,
    relative_path: &str,
    max_bytes: u64,
) -> io::Result<Vec<u8>> {
    ...
    while let Some(component) = components.next() {
        let component = std::ffi::CString::new(std::ffi::OsStr::new(component).as_bytes())
        ...
        // SAFETY: the retained parent descriptor is live and `component` is a
        // NUL-terminated direct child name. A successful descriptor is uniquely
        ...
        if final_component {
            ...
            if !metadata.file_type().is_file() {
                return Err(io::Error::new(
                    ...
                    "standard-library artifact is not a regular file",
                ));
            }
            if metadata.len() > max_bytes {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidData,
                    "standard-library artifact exceeds its byte limit",
                ));
            }
            let mut file = file;
            let mut bytes =
    ...
    Err(io::Error::new(
        io::ErrorKind::InvalidInput,
        "empty standard-library artifact path",
    ))
}
...
fn read_machine_std_no_follow_file(
    ...
    Err(io::Error::new(
        ...
        "descriptor-relative standard-library reads require Unix",
    ))
}
...
fn std_library_core_expr_hash(
    ...
    std_library_core_expr_payload_and_hash(loaded, owner, expr).map(|(_, hash)| hash)
}
...
fn std_library_core_expr_canonical_bytes(
    ...
    std_library_core_expr_payload_and_hash(loaded, owner, expr).map(|(payload, _)| payload)
}

fn std_library_core_level_hash(level: &Level) -> Result<Hash, MachineStdCanonicalBytesError> {
    std_library_core_level_payload_and_hash(level).map(|(_, hash)| hash)
}

fn std_library_core_expr_payload_and_hash(
    loaded: &MachineStdLoadedRelease,
    owner: &MachineStdLoadedModule,
    expr: &Expr,
) -> Result<(Vec<u8>, Hash), MachineStdCanonicalBytesError> {
    fn pointer(expr: &Expr) -> usize {
        std::ptr::from_ref(expr) as usize
    }

    let root = pointer(expr);
    let mut hashes = HashMap::<usize, Hash>::new();
    ...
    while let Some((expr, exiting)) = pending.pop() {
        ...
        let child_hash = |child: &Expr| {
            hashes
                .get(&pointer(child))
                .copied()
                .ok_or(MachineStdCanonicalBytesError::InvalidCoreExpressionGraph)
        };
        let mut payload = Vec::new();
        encode_std_library_core_expr_node(&mut payload, loaded, owner, expr, &child_hash)?;
        let hash = sha256_with_domain(b"NPA-TERM-0.1", &payload);
        if key == root {
            root_payload = Some(payload);
        }
        hashes.insert(key, hash);
    }

    let payload = root_payload.ok_or(MachineStdCanonicalBytesError::InvalidCoreExpressionGraph)?;
    let hash = hashes
        .remove(&root)
        .ok_or(MachineStdCanonicalBytesError::InvalidCoreExpressionGraph)?;
    Ok((payload, hash))
...
fn encode_std_library_core_expr_node(
    out: &mut Vec<u8>,
    loaded: &MachineStdLoadedRelease,
    owner: &MachineStdLoadedModule,
    expr: &Expr,
    child_hash: &impl Fn(&Expr) -> Result<Hash, MachineStdCanonicalBytesError>,
) -> Result<(), MachineStdCanonicalBytesError> {
    match expr {
        Expr::Sort(level) => {
            out.push(0x00);
        ...
        Expr::App(fun, arg) => {
            out.push(0x03);
            encode_hash(out, &child_hash(fun)?);
            encode_hash(out, &child_hash(arg)?);
        }
        Expr::Lam { ty, body, .. } => {
            out.push(0x04);
            encode_hash(out, &child_hash(ty)?);
            encode_hash(out, &child_hash(body)?);
        }
        Expr::Pi { ty, body, .. } => {
            out.push(0x05);
            encode_hash(out, &child_hash(ty)?);
...
fn std_library_core_level_payload_and_hash(
    level: &Level,
) -> Result<(Vec<u8>, Hash), MachineStdCanonicalBytesError> {
    enum Frame {
        Visit(Level),
    ...
    while let Some(frame) = pending.pop() {
        let mut payload = Vec::new();
        match frame {
            Frame::Visit(Level::Zero) => payload.push(0x00),
            Frame::Visit(Level::Param(name)) => {
                payload.push(0x04);
                encode_name(&mut payload, &Name::from_dotted(name))?;
            }
            ...
            Frame::Max | Frame::IMax => {
                ...
                let lhs = hashes
                    .pop()
                    .ok_or(MachineStdCanonicalBytesError::InvalidCoreExpressionGraph)?;
                payload.push(if matches!(frame, Frame::Max) {
                    0x02
                } else {
                    0x03
                });
                encode_hash(&mut payload, &lhs);
                encode_hash(&mut payload, &rhs);
            }
        }
        let hash = sha256_with_domain(b"NPA-LEVEL-0.1", &payload);
        hashes.push(hash);
        last_payload = Some(payload);
    }
    let payload = last_payload.ok_or(MachineStdCanonicalBytesError::InvalidCoreExpressionGraph)?;
    let hash = hashes
        .pop()
        .ok_or(MachineStdCanonicalBytesError::InvalidCoreExpressionGraph)?;
    if !hashes.is_empty() {
        return Err(MachineStdCanonicalBytesError::InvalidCoreExpressionGraph);
    }
    Ok((payload, hash))
}
...
fn sha256_with_domain(domain: &[u8], payload: &[u8]) -> Hash {
...
mod tests {
    ...
    #[test]
    fn standard_library_core_walkers_are_stack_safe_and_memoize_shared_dags() {
        let package = TestPackage::new("standard_library_core_walkers_stack_safe");
        write_valid_mvp_package(package.path());
        let loaded = load_machine_std_mvp_certificates(package.path()).unwrap();
        let owner = loaded.module(&Name::from_dotted("Std.Logic")).unwrap();

        let mut shallow = Arc::new(Expr::bvar(0));
        for _ in 0..8 {
            shallow = Arc::new(Expr::App(Arc::clone(&shallow), Arc::clone(&shallow)));
        }
        let shallow_hash = std_library_core_expr_hash(&loaded, owner, &shallow).unwrap();
        assert_eq!(
            shallow_hash,
            std_library_core_expr_hash(&loaded, owner, &shallow).unwrap()
        );

        let mut deep = shallow;
        for _ in 0..8_192 {
            deep = Arc::new(Expr::App(Arc::clone(&deep), Arc::clone(&deep)));
        }
        assert_eq!(syntactic_expr_size(&deep), u64::MAX);
        assert!(!expr_has_bvar_at_or_above(&deep, 1));
        assert_eq!(
            std_library_core_expr_canonical_bytes(&loaded, owner, &deep)
                .unwrap()
                .len(),
            65
        );
        assert_ne!(
            std_library_core_expr_hash(&loaded, owner, &deep).unwrap(),
            [0; 32]
        );
        let mut heads = BTreeMap::new();
        collect_expr_heads(&loaded, owner, &deep, &mut heads).unwrap();
        ...
        assert_ne!(std_library_core_level_hash(&level).unwrap(), [0; 32]);
        std::mem::forget(level);

        let max_index_under_new_binder =
            Expr::lam("x", Expr::sort(Level::zero()), Expr::bvar(u32::MAX));
        assert!(!expr_has_bvar_at_or_above(
            &max_index_under_new_binder,
            u32::MAX
        ));
    }
    ...
    fn rejects_core_or_prelude_import_entry_as_unresolved_release_import() {
        for imported in ["Core", "Prelude"] {
            ...
            let logic = decode_module_cert(certs.logic.as_slice()).unwrap();
            let mut parts = logic.into_parts();
            parts.imports.push(ImportEntry {
                module: Name::from_dotted(imported),
                export_hash: [0; 32],
                certificate_hash: Some([0; 32]),
            });
            let logic = ModuleCert::from_parts(parts);
    ...
    fn rejects_release_import_cycles_before_verification() {
        ...
        let mut certs = mvp_certificate_bytes();
        let logic = decode_module_cert(certs.logic.as_slice()).unwrap();
        let nat = decode_module_cert(certs.nat.as_slice()).unwrap();
        let mut logic_parts = logic.into_parts();
        let mut nat_parts = nat.into_parts();
        logic_parts.imports.push(ImportEntry {
            module: Name::from_dotted("Std.Nat"),
            export_hash: nat_parts.hashes.export_hash,
            certificate_hash: Some(nat_parts.hashes.certificate_hash),
        });
        nat_parts.imports.push(ImportEntry {
            module: Name::from_dotted("Std.Logic"),
            export_hash: logic_parts.hashes.export_hash,
            certificate_hash: Some(logic_parts.hashes.certificate_hash),
        });
        let logic = ModuleCert::from_parts(logic_parts);
        let nat = ModuleCert::from_parts(nat_parts);
        certs.logic = encode_module_cert(&logic).unwrap();
    ...
    #[cfg(unix)]
    #[test]
    fn rejects_in_tree_symlink_components_and_oversized_certificates() {
        use std::os::unix::fs::symlink;

        let package = TestPackage::new("symlink_component");
        let outside = TestPackage::new("symlink_component_outside");
        let certs = mvp_certificate_bytes();
        write_mvp_package(package.path(), &certs);
        fs::write(outside.path().join("Basic.npcert"), &certs.algebra_basic).unwrap();
        fs::remove_file(package.path().join(STD_ALGEBRA_BASIC_PATH)).unwrap();
        fs::remove_dir(package.path().join("Std/Algebra")).unwrap();
        symlink(outside.path(), package.path().join("Std/Algebra")).unwrap();

        assert!(matches!(
            load_machine_std_mvp_certificates(package.path()).unwrap_err(),
            MachineStdReleaseLoaderError::ReadCertificateFile { .. }
        ));

        fs::remove_file(package.path().join("Std/Algebra")).unwrap();
        fs::create_dir(package.path().join("Std/Algebra")).unwrap();
        let oversized = File::create(package.path().join(STD_ALGEBRA_BASIC_PATH)).unwrap();
        oversized.set_len(67_108_865).unwrap();
        assert!(matches!(
            load_machine_std_mvp_certificates(package.path()).unwrap_err(),
            MachineStdReleaseLoaderError::ReadCertificateFile { .. }
        ));
    ...
    #[cfg(unix)]
    #[test]
    fn release_loader_rejects_symlinked_and_oversized_json_artifacts() {
        use std::os::unix::fs::symlink;

        let package = TestPackage::new("release_json_boundaries");
        let outside = TestPackage::new("release_json_boundaries_outside");
        write_valid_mvp_package(package.path());
        let loaded = load_machine_std_mvp_certificates(package.path()).unwrap();
        ...
        let import_bundles_path =
            join_posix_relative_path(package.path(), STD_MACHINE_IMPORT_BUNDLES_JSON_PATH);
        fs::write(
            outside.path().join("import-bundles.json"),
            import_bundle_set_json(&import_bundles),
        )
        .unwrap();
        fs::remove_file(&import_bundles_path).unwrap();
        symlink(
            outside.path().join("import-bundles.json"),
            &import_bundles_path,
        )
        .unwrap();
        assert!(matches!(
            load_machine_std_mvp_release(package.path()).unwrap_err(),
            MachineStdReleaseArtifactError::ReadArtifact {
                artifact: MachineStdArtifactKind::ImportBundles,
                ..
            }
        ));
        ...
        .unwrap();
        let release_path = join_posix_relative_path(package.path(), STD_MACHINE_RELEASE_JSON_PATH);
        let oversized = File::create(&release_path).unwrap();
        oversized.set_len(MAX_MACHINE_STD_JSON_BYTES + 1).unwrap();
        assert!(matches!(
            load_machine_std_mvp_release(package.path()).unwrap_err(),
            MachineStdReleaseArtifactError::ReadArtifact {
                artifact: MachineStdArtifactKind::LibraryRelease,
                ..
            }
        ));
    }
...
theorem list_map_id_inferred (A : Type) (xs : List.{1} A) : Eq.{1} (List.{1} A) (List.map.{1,1} A A (fun (a : A) => a) xs) xs := by
  ...
  exact List.map_id A xs",
    ...
    fn source_package_build_fails_on_import_hash_mismatch() {
        ...
        let nat = decode_module_cert(&certs.nat).unwrap();
        let mut parts = nat.into_parts();

        parts.imports[0].export_hash = test_hash(201);
        ...
        parts.imports[0].export_hash = logic.export_hash();
        parts.imports[0].certificate_hash = Some(test_hash(202));
        let err = validate_source_build_import_entries(
            &Name::from_dotted("Std.Nat"),
            &parts.imports,
            std::slice::from_ref(&logic),
    ...
    #[test]
    fn std_library_projection_one_table() {
        let certs = mvp_certificate_bytes();
        let mut session = VerifierSession::new();
        ...
        .unwrap();
        let first = npa_cert::verified_module_to_kernel_decls(&verified).unwrap();
        let second = npa_cert::verified_module_to_kernel_decls(&verified).unwrap();
        assert_eq!(first, second);
        assert_eq!(first.len(), verified.declarations().len());
        assert!(first.iter().any(|decl| decl.name() == "Eq"));
    }
