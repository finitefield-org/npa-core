...
if [ "$CURRENT_VERSION" != 0.8.0 ]; then
  echo "toolchain v0.8 compatibility requires npa-cli 0.8.0, found $CURRENT_VERSION" >&2
  exit 1
fi
MODE=full
...
phase "locked offline host 0.8 metadata, build, API, and adapter gates"
...
cargo test --locked --offline --manifest-path "$ROOT/Cargo.toml" -p npa-cli \
  --example inspect_ext_v0_8_policy
cargo test --locked --offline --manifest-path "$ROOT/Cargo.toml" -p npa-cli \
  --test package_api_v1
cargo test --locked --offline --manifest-path "$ROOT/Cargo.toml" -p npa-cli \
...
phase "deterministic fixture and v0.8 policy preflight"
...
if [ "$MODE" = full ]; then
  ...
  if [ -n "$FINAL_STATUS" ]; then
    echo "full toolchain v0.8 compatibility changed the candidate checkout" >&2
    printf '%s\n' "$FINAL_STATUS" >&2
    exit 1
  fi
fi
...
if [ "$MODE" = functional ]; then
  echo "toolchain v0.8 functional compatibility passed; release evidence not evaluated"
else
  echo "toolchain v0.8.0 compatibility and release evidence passed"
fi
