...
do
  if ! command -v "$tool" >/dev/null 2>&1; then
    echo "toolchain v0.6 compatibility requires $tool" >&2
    exit 1
  fi
done
if [ "$MODE" = full ]; then
  ...
  do
    if ! command -v "$tool" >/dev/null 2>&1; then
      echo "full toolchain v0.6 compatibility requires $tool" >&2
      exit 1
    fi
  done
fi
...
if [ "$MODE" = full ] && [ -n "$(/usr/bin/git -C "$GIT_ROOT" status --porcelain=v1 --untracked-files=all)" ]; then
  echo "full toolchain v0.6 compatibility requires a clean candidate checkout" >&2
  exit 1
fi
...
RUN_PARENT="$ROOT/target/npa-checker-ext-v0.6"
mkdir -p "$RUN_PARENT"
...
phase "locked offline host 0.6 metadata, build, API, and adapter gates"
...
cargo test --locked --offline --manifest-path "$ROOT/Cargo.toml" -p npa-cli \
  --example verify_ext_v0_6_facade
cargo test --locked --offline --manifest-path "$ROOT/Cargo.toml" -p npa-cli \
  --example inspect_ext_v0_6_policy
cargo test --locked --offline --manifest-path "$ROOT/Cargo.toml" -p npa-cli \
  --test package_api_v1
cargo test --locked --offline --manifest-path "$ROOT/Cargo.toml" -p npa-cli \
...
phase "deterministic fixture and v0.6 policy preflight"
...
if [ "$MODE" = full ]; then
  ...
  if [ -n "$FINAL_STATUS" ]; then
    echo "full toolchain v0.6 compatibility changed the candidate checkout" >&2
    printf '%s\n' "$FINAL_STATUS" >&2
    exit 1
  fi
fi
...
if [ "$MODE" = functional ]; then
  echo "toolchain v0.6 functional compatibility passed; release evidence not evaluated"
else
  echo "toolchain v0.6.0 compatibility and release evidence passed"
fi
